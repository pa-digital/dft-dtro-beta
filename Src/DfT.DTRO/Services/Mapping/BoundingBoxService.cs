...
public class BoundingBoxService : IBoundingBoxService
{
    ...
    public BoundingBox SetBoundingBoxForSingleGeometry(List<SemanticValidationError> errors, JProperty jProperty, BoundingBox boundingBox)
    {
        ...
        {
            case "PointGeometry":
                items = geometrySelected
                    .DescendantsAndSelf()
                    .ElementAtOrDefault((int)GeometryType.PointGeometry)
                    .ToIndentedJsonString();

                //json = geometrySelected.DescendantsAndSelf().Skip(3).FirstOrDefault().ToIndentedJsonString();
                if (!items.Contains(Constants.Srid27000))
                {
            ...
            case "LinearGeometry":
                items = geometrySelected
                    .DescendantsAndSelf()
                    .ElementAtOrDefault((int)GeometryType.LinearGeometry)
                    .ToIndentedJsonString();
                if (!items.Contains(Constants.Srid27000))
                {
                    errors.Add(Error.SetSetSpatialReferenceValidationError());
            ...
            case "Polygon":
                items = geometrySelected
                    .DescendantsAndSelf()
                    .ElementAtOrDefault((int)GeometryType.Polygon)
                    .ToIndentedJsonString();
                if (!items.Contains(Constants.Srid27000))
                {
                    errors.Add(Error.SetSetSpatialReferenceValidationError());
            ...
            case "DirectedLinear":
                items = geometrySelected
                    .DescendantsAndSelf()
                    .ElementAtOrDefault((int)GeometryType.DirectedLinear)
                    .ToIndentedJsonString();
                if (!items.Contains(Constants.Srid27000))
                {
                    errors.Add(Error.SetSetSpatialReferenceValidationError());
